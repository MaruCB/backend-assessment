...
def test_write_file(mock_open: MagicMock, mock_os: MagicMock) -> None:
    ...
    user_list = []
    new_user = User(company_id=1, surname="ras", location="London", forename="Ras", date_of_birth="1990/01/31")
    new_user.full_name = "Ras ras"
    company = Company(headquarters="Nellie", id=1, name="Wonderland", industry="Fluffyness")
...
def test_write_file_remove(mock_open: MagicMock, mock_os: MagicMock) -> None:
    ...
    new_user = User(company_id=1, surname="ras", location="London", forename="Ras", date_of_birth="1990/01/31")
    new_user.full_name = "Ras ras"
    company = Company(headquarters="Wonderlan", id=1, name="Nellie", industry="Fluffyness")
    new_user.company = company
    user_list.append(new_user)
